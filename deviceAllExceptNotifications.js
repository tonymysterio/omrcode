...
request.onload = function() {
  if (request.status >= 200 && request.status < 400) {
    ...
    //
    //let dataWithStatuses = grouper.filterNonStatuses(data,{info:false});
    let groupByInstanceID = grouper.groupBy_user_dim_app_info_app_instance_id(data);
    let pgs = grouper.processGroups(groupByInstanceID); //returns populated groups, omit info

    let groupsWithStatuses = grouper.filterNonStatusesFromGroups(pgs[1],{info:false});
    //let pgs = grouper.processGroups(dataWithStatuses); //returns populated groups, omit info
    let lev3 = grouper.prepareForLevel3Report(groupsWithStatuses);
    ...
    let gmgm = grouper.lookForStatusInGroups(pgs[1]);

    let fnaa = grouper.EventsErrorsEtcAcrossPopulatedGroups(pgs[1]);
    console.log(fnaa);
    let fi = grouper.deviceAllExceptNotifications(fnaa);
    displayTable( fi );
  } else {
